...
| `--shape {models\|dicts}` | unset (dual shape) | Lock the generated client to a single response shape. Omit to produce a dual-shape client (`shape=` constructor + `with_shape()`). `--shape dicts` also skips `base/models.py`. See [Response shape](../user-guide/client-usage.md#response-shape-models-vs-dicts). |
